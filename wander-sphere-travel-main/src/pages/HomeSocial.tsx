...
import { Plus, Camera, Heart, MessageCircle, Share2, Bookmark, MoreHorizontal, MapPin, X, ChevronLeft, ChevronRight, Send, Flag, UserX, Smile, AtSign, Hash, Search, Link, Palette, AlignLeft, AlignCenter, AlignRight, Compass, Film, BellOff, Layers, Home as HomeIcon, CheckCircle, Play, Loader2, Luggage, Users, Map, Plane } from "lucide-react";
...
const AddStoryDialog: FC<AddStoryDialogProps> = React.memo(({ open, onOpenChange }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // Upload to Bunny.net first (Search & Destroy Mission: No more Supabase Storage!)
            console.log('📤 Uploading story to Bunny.net...');
            const uploadResult = await uploadToBunny(selectedFile, 'story', {
                compress: mediaType === 'image', // Only compress images
                quality: 0.85,
                onProgress: (progress) => {
            ...
            if (!uploadResult.success || !uploadResult.publicUrl) {
                throw new Error(uploadResult.error || 'Failed to upload story to Bunny.net');
            }

            console.log('✅ Story uploaded:', uploadResult.publicUrl);
            
            // Create story record with URL
            const storyData = {
                mediaType: mediaType as 'image' | 'video',
                duration: mediaType === 'video' ? 15 : undefined,
            };
            
            // Pass URL as second argument (mediaUrl)
            const response = await storyService.createStory(storyData, uploadResult.publicUrl);
            ...
            if (response.success) {
                ...
                throw new Error(response.message || 'Failed to create story record');
            }
        } catch (error: any) {
...
const Home: FC = () => {
  ...
  const fetchClubs = useCallback(async () => {
    try {
      ...
      const response = await clubService.getClubs({ limit: 5 }); // Fetch first 5 clubs for home
      if (response && response.data) {
        // @ts-ignore - Handling potential PaginatedResponse structure flexibility
        setClubs(Array.isArray(response.data) ? response.data : response.data.data || []); 
      } else {
        setClubs([]);
      }
    } catch (error) {
      console.error('Failed to fetch clubs:', error);
      // Don't show toast for this non-critical section
    } finally {
    ...
    try {
      ...
      if (response && (response.success || response.data)) {
         ...
         const transformedPosts = Array.isArray(journeys) ? journeys.map((journey: any) => {
           ...
           return {
             id: postId || journey.id?.toString(), // Keep as string for UUID compatibility
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted/20 relative">
      ...
      <div className="flex-1 overflow-y-auto pt-4 lg:pt-6 pb-20 lg:pb-6">
        <div className="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto px-3 sm:px-4 md:px-6 py-2 md:py-3">
          ...
          {activeTab === 'feed' && (
            <div className="space-y-4 lg:space-y-6">
              ...
              {/* Quick Access Section - Replaces Travel Clubs */}
              {/* Quick Actions - Compact Row */}
              <div className="grid grid-cols-3 gap-3 px-1 my-4">
                {/* Trips */}
                <div 
                  onClick={() => navigate('/find-trips')}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 flex flex-col items-center justify-center gap-1 h-20 active:scale-95 transition-all cursor-pointer hover:shadow-md"
                >
                  <div className="bg-blue-50 p-2 rounded-full">
                    <Plane className="w-5 h-5 text-blue-600" />
                  </div>
                  <span className="text-xs font-semibold text-gray-700">Trips</span>
                </div>
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-gray-700">Clubs</span>
                </div>
