...
const BookingSpace = () => {
  ...
  const fetchBookingPartners = useCallback(async () => {
    try {
      ...
      // const response = await apiRequest(`${apiConfig.baseURL}${endpoints.bookingPartners}`);
      // setBookingPartners(response.data);
      
      // For now, set empty array to show "no partners" state
      setBookingPartners([]);
    } catch (error) {
  ...
  const fetchFeatures = useCallback(async () => {
    try {
      // TODO: Replace with actual API call
      // const response = await apiRequest(`${apiConfig.baseURL}${endpoints.bookingFeatures}`);
      // setFeatures(response.data);
      
      // For now, set empty array to show "no features" state
      setFeatures([]);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            bookingPartners.map((partner, index) => (
              <Card key={index} className="overflow-hidden hover:shadow-lg transition-all duration-300 bg-surface-elevated">
                <CardHeader className="text-center pb-4">
                ...
                <CardContent className="text-center">
                  <p className="text-primary font-medium mb-2">{partner.type}</p>
